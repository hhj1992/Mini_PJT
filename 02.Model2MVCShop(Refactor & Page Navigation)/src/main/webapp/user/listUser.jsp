...
<script type="text/javascript">
	...
	function fncGetUserList(currentPage) {
		...
		//currentPage라는 name값을 가진 value값에 매개변수로 받아온 currentPage값을 세팅
	   	...
	   	//현재 detailForm에는 searchCondition 값과 searchKeyword가 있기때문에 submit시킬때 action(/listUser.do)로 전송 
	}
</script>
	...
	<tr>
		<td align="right">
			...
			<select name="searchCondition" class="ct_input_g" style="width:80px">
				<option value="0" <%= (searchCondition.equals("0") ? "selected" : "")%>>회원ID</option> <!-- searchCondition값이 0이면 회원ID를 select -->
				<option value="1" <%= (searchCondition.equals("1") ? "selected" : "")%>>회원명</option>  <!-- searchConditionn값이 1이면 회원명을 select -->
			</select>
			<input 	type="text" name="searchKeyword" value="<%= searchKeyword %>"  class="ct_input_g"  style="width:200px; height:20px" ><!-- searchKeyword값을 value값에 setting -->
		</td>
